<?php
	...
	$acr=true;
	$renew = true;
	$checkquery = " SELECT * FROM `hatchery` WHERE `ID`='$ID'";
	...
	{
		if ($checkpost['Acc DFO']!='')
		{	
			if($checkpost['Acc DFO']=='canceled'){
				$cancel = true;
			}
			else if($checkpost['Acc ADF']=='canceled'){
				$cancel = true;
			}
			else if($checkpost['Acc JDF']=='canceled'){
				$cancel = true;
			}
			$check=false;
		}
		if ($checkpost['Acc JDF']!='')
		{
			$acr=false;
		}
		$date1=date_create($checkpost['Acc Valid']);
	...
	{	
		...
		{	
			...
			upload('Photo3');
			upload('Photo4');
			header("Location: http://localhost/hatchery_project/Project1/details.php");
			var_dump($query);
		}
		else
		{
  ...
  <div class="Details">
	...
	<?php
		...
		{	
			...
			{
				echo "	<br>
					<p class='error'>Your Form has been canceled<br>
					<span style='color: red; font-size:smaller'>Reason : ".$checkpost['Remarks']."</span>
					</p>
				";
			}
			else if ($acr == false)
			{
				echo "	<br>
					<p class='error'>Accreditation NO. : ".$checkpost['Acc No']."</p>
				";
			}
			else
			{
				echo "	<br>
				<p class='error'>Accreditation under process... </p>
			";
			}
			if ($renew==false)
			{
				echo "
				<p class='error blink'>Go to Renewal and renew</p>
			";
			}
		}
		elseif ($nodata == true)
		...
		{
			echo"	<br>
				<p class='error'>Accrediation Process not started, Data can be modified
			";
		}
	?>
	<div class="bg0">
