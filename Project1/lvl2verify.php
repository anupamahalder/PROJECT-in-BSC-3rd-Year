<?php
	...
	if (empty($_POST['ID'])) {
		...
		$ID = 0;
		$IDquery="SELECT MIN(`ID`) AS 'curr' FROM `hatchery` WHERE `ID`>='$ID' AND `Current`='Y' AND `Hatchery District`='$currDistrict' AND `ACC ADF`='' AND `ACC DFO`!='' AND `ACC DFO`!='canceled';";
	...
	{
		$IDquery="SELECT MAX(`ID`) AS 'curr' FROM `hatchery` WHERE `ID`<'$ID' AND `Current`='Y' AND `Hatchery District`='$currDistrict' AND `ACC ADF`='' AND `ACC DFO`!='' AND `ACC DFO`!='canceled';";
		$IDresult = mysqli_query($conn,$IDquery);
		$IDposts = mysqli_fetch_all($IDresult,MYSQLI_ASSOC);
		mysqli_free_result($IDresult);
	...
	if (filter_has_var(INPUT_POST, 'Verify'))
	{
		...
		$date = $_POST['date'];
		var_dump($_POST);
		$authquery = "UPDATE `hatchery` SET `Acc ADF` = '$authBy',`ADF date` = '$date' WHERE `ID` = '$ID'";
		if( mysqli_query($conn,$authquery))
		{	
			$maxIDquery="SELECT MAX(`ID`) AS 'maxid' FROM `hatchery` WHERE `Current`='Y' AND `Hatchery District`='$currDistrict' AND `ACC ADF`='' AND `ACC DFO`!='' AND `ACC DFO`!='canceled';";
			...
			if ($max = true) {
				header("Location: ./lvl2verify.php");
			}
			else{
				$IDquery="SELECT MIN(`ID`) AS 'curr' FROM `hatchery` WHERE `ID`>'$ID' AND `Current`='Y' AND `Hatchery District`='$currDistrict' AND `ACC ADF`='' AND `ACC DFO`!='' AND `ACC DFO`!='canceled';";
				$IDresult = mysqli_query($conn,$IDquery);
	...
	{	
		...
		$authquery = "UPDATE `hatchery` SET `Acc ADF` = 'canceled',`ADF date` = '$date',`Remarks` = '$remarks' WHERE `ID` = '$ID'";
		echo $authquery;
		if( mysqli_query($conn,$authquery))
		{	
			$maxIDquery="SELECT MAX(`ID`) AS 'maxid' FROM `hatchery` WHERE `Current`='Y' AND `Hatchery District`='$currDistrict' AND `ACC ADF`='' AND `ACC DFO`!='' AND `ACC DFO`!='canceled';";
	...
	$minIDquery="SELECT MIN(`ID`) AS 'minid' FROM `hatchery` WHERE `Current`='Y' AND `Hatchery District`='$currDistrict'  AND `ACC ADF`='' AND `ACC DFO`!='' AND `ACC DFO`!='canceled';";
...
<?php
    ...
    <li>
     ...
     <li>
       <a href="Search.php">
         ...
         <span class="links_name">Search Hatchery</span>
       </a>
       <span class="tooltip">Search Hatchery</span>
     </li>
     <li>
  ...
  <section class="home-section">
	<div class="inform">
	<form method="POST" action="lvl2verify.php">
