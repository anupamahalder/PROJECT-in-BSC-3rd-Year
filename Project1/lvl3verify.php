<?php
	...
	if (empty($_POST['ID'])) {
		...
		$ID = 0;
		$IDquery="SELECT MIN(`ID`) AS 'curr' FROM `hatchery` WHERE `ID`>='$ID' AND `Current`='Y'  AND `ACC ADF`!='' AND `ACC JDF`='' AND `ACC ADF`!='canceled' AND `ACC DFO`!='' AND `ACC DFO`!='canceled';";
	...
	{
		$IDquery="SELECT MAX(`ID`) AS 'curr' FROM `hatchery` WHERE `ID`<'$ID' AND `Current`='Y'   AND `ACC ADF`!='' AND `ACC JDF`='' AND `ACC ADF`!='canceled' AND `ACC DFO`!='' AND `ACC DFO`!='canceled';";
		$IDresult = mysqli_query($conn,$IDquery);
		$IDposts = mysqli_fetch_all($IDresult,MYSQLI_ASSOC);
		mysqli_free_result($IDresult);
	...
	if (filter_has_var(INPUT_POST, 'Verify'))
	{
		...
		$oldquery="SELECT * FROM `hatchery` WHERE `ID`='old$ID' AND `Current`='N';";
		$oldresult = mysqli_query($conn,$oldquery);
		$oldposts = mysqli_fetch_all($oldresult,MYSQLI_ASSOC);
		mysqli_free_result($oldresult);

		if (!empty($oldposts[0])) 
		{
			$acc = $oldposts[0]['Acc No'];
		}
		else
		{
			$Accquery="SELECT MAX(`Acc No`) AS 'accMax' FROM `hatchery`";
			$Accresult = mysqli_query($conn,$Accquery);
			$Accposts = mysqli_fetch_all($Accresult,MYSQLI_ASSOC);
			mysqli_free_result($Accresult);
			if (empty($Accposts[0]['accMax'])){
				$acc = 'ACR-1';
			}
			else{
				$acctemp = $Accposts[0]['accMax'][-1]+1;
				$acc='ACR-'.$acctemp;
			}
		}
		$authquery = "UPDATE `hatchery` SET `Acc JDF` = '$authBy',`JDF date` = '$date' WHERE `ID` = '$ID'";
		$accquery = "UPDATE `hatchery` SET `Acc No` = '$acc',`Acc Date` = '$date' WHERE `ID` = '$ID'";
		if( mysqli_query($conn,$authquery))
		{	
			if(mysqli_query($conn,$accquery))
			{
			...
			$maxIDquery="SELECT MAX(`ID`) AS 'maxid' FROM `hatchery` WHERE `Current`='Y' AND `ACC ADF`='' AND `ACC DFO`!='' AND `ACC DFO`!='canceled';";
			...
			if ($max = true) {
				header("Location: ./lvl3verify.php");
			}
			else{
				$IDquery="SELECT MIN(`ID`) AS 'curr' FROM `hatchery` WHERE `ID`>'$ID' AND `Current`='Y'   AND `ACC ADF`!='' AND `ACC JDF`='' AND `ACC ADF`!='canceled' AND `ACC DFO`!='' AND `ACC DFO`!='canceled';";
				$IDresult = mysqli_query($conn,$IDquery);
	...
	{	
		...
		$authquery = "UPDATE `hatchery` SET `Acc JDF` = 'canceled',`JDF date` = '$date',`Remarks` = '$remarks' WHERE `ID` = '$ID'";
		echo $authquery;
		if( mysqli_query($conn,$authquery))
		{	
			$maxIDquery="SELECT MAX(`ID`) AS 'maxid' FROM `hatchery` WHERE `Current`='Y'   AND `ACC ADF`!='' AND `ACC JDF`='' AND `ACC ADF`!='canceled' AND `ACC DFO`!='' AND `ACC DFO`!='canceled';";
			$maxIDresult = mysqli_query($conn,$maxIDquery);
			$maxIDposts = mysqli_fetch_all($maxIDresult,MYSQLI_ASSOC);
			mysqli_free_result($IDresult);
			if ($maxIDposts[0]['maxid']==$currentID){
	...
	$minIDquery="SELECT MIN(`ID`) AS 'minid' FROM `hatchery` WHERE `Current`='Y'    AND `ACC ADF`!='' AND `ACC JDF`='' AND `ACC ADF`!='canceled' AND `ACC DFO`!='' AND `ACC DFO`!='canceled';";
...
<?php
  ...
  <section class="home-section">
	<div class="inform">
	<form method="POST" action="lvl3verify.php">
